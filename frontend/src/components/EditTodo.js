...
const InputFormName = styled.input`
  font-size: 20px;
  width: 100%;
  height: 40px;
  padding: 2px 7px;
...
function EditTodo(props) {
  ...
  const getTodo = id => {
    axios.get(`http://localhost:3001/api/v1/todos/${id}`)
    .then(resp => {
  ...
  const updateTodo = () => {
    axios.patch(`http://localhost:3001/api/v1/todos/${currentTodo.id}`, currentTodo)
    .then(response => {
      notify();
  ...
  const deleteTodo = () => {
    ...
    if (sure) {
      axios.delete(`http://localhost:3001/api/v1/todos/${currentTodo.id}`)
      .then(resp => {
